let cars = [
    ...
    },
    {
        ...
        "barter": true,
        "images": [
            "https://turbo.azstatic.com/uploads/f460x343/2024%2F05%2F04%2F17%2F12%2F41%2F08bfc957-79e4-470f-9aa9-5e5d68c536c5%2F64677_XEKjwmAveF1J7d42VxCbsw.jpg"
        ],
        "city": "Bakı",
        "engine": 1.5
    },
...
const vBtns = document.querySelectorAll('#vezyet-btn button')
const valBtns = document.querySelectorAll('#valyuta-desk button')
const markaDesk = document.getElementById('marka-desk')
const modelDesk = document.getElementById('model-desk')
const banDesk = document.getElementById('ban-desk')
const val = document.getElementById('val')
const cityDesk = document.getElementById('city-desk')
const yearminbtn = document.getElementById('year-min-btn')
const yearmaxbtn = document.getElementById('year-max-btn')
const minPrice = document.getElementById('min-price')
const maxPrice = document.getElementById('max-price')
const ibrahimList = document.getElementById('ibrahim-list')

const marka = [... new Set(cars.map(item => item.brand))]
const city = [... new Set(cars.map(item => item.city))]
const currency = [... new Set(cars.map(item => item.currency))]
const ban = [... new Set(cars.map(item => item.banType))]
...
function showElan() { 
    document.querySelector(".nav-accordion").classList.toggle('nav-accordion-active') 
    if (!flag) {
        document.querySelector(".nav-acc-ac").innerHTML = `Gizlət <i class="fa-solid fa-chevron-up"></i>`
        flag = true
    }else{
        document.querySelector(".nav-acc-ac").innerHTML = `Daha çox filtr <i class="fas fa-chevron-down"></i>`
        flag =false
    }
}

marka.map((item) => markaDesk.innerHTML += `<option value="${item}">${item}</option>`)
city.map((item) => cityDesk.innerHTML += `<option value="${item}">${item}</option>`)
currency.map((item) => val.innerHTML += `<option value="${item}">${item}</option>`)
ban.map((item) => banDesk.innerHTML += `<option value="${item}">${item}</option>`)

function ModelSec() {
    kod = `<option value="Model">Model</option>`
    modelDesk.classList.remove('model-desk')
    modelDesk.innerHTML = `<option value="Model">Model</option>`;
    let models = [...new Set(cars.filter(item => markaDesk.value == item.brand).map(item => item.model))]
    for (let i = 0; i < models.length; i++) kod += `<option>${models[i]}</option>`
    modelDesk.innerHTML = kod
}

function VbtnColor(z) {
    for (let i = 0; i < vBtns.length; i++) vBtns[i].classList.remove('vezyet-btn-active')
    vBtns[z].classList.add('vezyet-btn-active')
}

for (let i = 2024; i > 1903; i--) {
    yearmaxbtn.innerHTML += `<option>${i}</option>`
    yearminbtn.innerHTML += `<option>${i}</option>`
}

function show() { cars.map((item) => { main.innerHTML += CarCard(item) }) }
show()

function valBtnsactive(x) {
    valBtns[x].classList.toggle('val-active')
}

function emil(){
    markalarMobile.map((item, i) =>{
        ibrahimList.innerHTML += `<li onclick="showCarsModel(${i})">
            <img src="${item.img}" />
            <span id="marka-names">${item.markaM}</span>
        </li>`
    })
}
emil()
...
function showCarsModel(a) {
    const markaTap = document.querySelectorAll("#marka-names")
    footer.style.display = 'block'
    main.style.display = 'flex'
    markaPage.style.display = 'none'
    markaflag = false
    main.innerHTML = ''
    let filtered = cars
...
function CarCard(element) {
    ...
    return   `<div id="a${element.id}" class="card">
                <div class="card-image">
                    <img height="100%" width="100%" src="${element.images}" />
                </div>
                <div class="price">
                    <span class="card-qiymet">${element.price}</span> <span class="card-val">${element.currency}</span>
                </div>
                <div class="name">
                    <span class="card-brand">${element.brand}</span>  <span class="card-model">${element.model}</span>
                </div>
                <div class="info">
                    <span class="card-year">${element.year}</span>, 
                    <span class="card-engine">${element.engine}</span>L, 
                    <span class="card-yurus">${element.odometer}</span> 
                    <span class="card-mil">${element.odometerUnit}</span>
                </div>
                <span class="konum">${element.city}</span><span class="card-date">, bugün 15:51</span>
                <span class="${krd}"><i class="fa-solid fa-percent"></i></span>
                <span class="${krd1}"><i class="fa-solid fa-rotate"></i></span>
            </div>`
}
function showFilter() {
    main.innerHTML = ''
    ...
    if (markaDesk.value !== "Marka") {
        filtered = filtered.filter((car) => {
            return car.brand === markaDesk.value
        })
    }
    

    if (modelDesk.value !== "Model") {
        filtered = filtered.filter((car) => {
            return car.model === modelDesk.value
        })
    }
    
    if (cityDesk.value !== "Şəhər") {
        filtered = filtered.filter((car) => {
            return car.city === cityDesk.value
        })
    }

    if (banDesk.value !== "Ban növü") {
        filtered = filtered.filter((car) => {
            return car.banType === banDesk.value
        })
    }

    if (minPrice.value !== "") {
        filtered = filtered.filter((car) => {
            return car.price > minPrice.value
        })
    }

    if (maxPrice.value !== "") {
        filtered = filtered.filter((car) => {
            return car.price < maxPrice.value
        })
    }
    
    if (val.value !== "") {
        filtered = filtered.filter((car) => {
            return car.currency === val.value
        })
    }

    if (yearminbtn.value !== "") {
        filtered = filtered.filter((car) => {
            return car.year > yearminbtn.value
        })
    }

    if (yearmaxbtn.value !== "") {
        filtered = filtered.filter((car) => {
            return car.year < yearmaxbtn.value
        })
    }

    if (valBtns[0].classList.contains("val-active")) {
        filtered = filtered.filter((car) => {
            return car.credit
        })
    }

    if (valBtns[1].classList.contains("val-active")) {
        filtered = filtered.filter((car) => {
            return car.barter
        })
    }
    ...
    if (filtered.length) {
        filtered.forEach((car) => {
            main.innerHTML += CarCard(car)
        })
    } else {
        main.innerHTML += "Bu viddə maşın yoxdur !!"
    }
}
const footer = document.querySelector('footer')
const markaPage = document.getElementById('marka-page')
function markaPageBtn(){
    if (!markaflag) {
        footer.style.display = 'none'
        main.style.display = 'none'
        markaPage.style.display = 'block'
        markaflag = true
    }else{
        footer.style.display = 'block'
        main.style.display = 'flex'
